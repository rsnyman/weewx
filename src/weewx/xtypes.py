...
class ArchiveTable(XType):
    ...
    def get_series(obs_type, timespan, db_manager, aggregate_type=None, aggregate_interval=None,
        ...
        if aggregate_type:
            ...
            # No aggregation
            sql_str = "SELECT dateTime, %s, usUnits, %s FROM %s " \
                      "WHERE dateTime > ? AND dateTime <= ?" % (
                          obs_type, db_manager.quote('interval'), db_manager.table_name)
    ...
    agg_sql_dict = {
        ...
        'vecdir': "SELECT SUM(%(interval)s * windSpeed * COS(RADIANS(90 - windDir))), "
                  "       SUM(%(interval)s * windSpeed * SIN(RADIANS(90 - windDir))) "
                  "FROM %(table_name)s "
                  "WHERE dateTime > %(start)s AND dateTime <= %(stop)s ",
        'vecavg': "SELECT SUM(%(interval)s * windSpeed * COS(RADIANS(90 - windDir))), "
                  "       SUM(%(interval)s * windSpeed * SIN(RADIANS(90 - windDir))), "
                  "       SUM(%(interval)s) "
                  "FROM %(table_name)s "
                  "WHERE dateTime > %(start)s AND dateTime <= %(stop)s "
    ...
    def get_aggregate(obs_type, timespan, aggregate_type, db_manager, **option_dict):
        ...
        interpolate_dict = {
            'interval': db_manager.quote('interval'),
    ...
    def get_wind_aggregate_long(obs_type, timespan, aggregate_type, db_manager):
        ...
        sql_stmt = "SELECT %(interval)s, windSpeed, windDir " \
                   "FROM %(table_name)s " \
                   "WHERE dateTime > %(start)s AND dateTime <= %(stop)s;" \
                   % {
                       'interval': db_manager.quote('interval'),
...
class WindVec(XType):
    ...
    def get_series(obs_type, timespan, db_manager, aggregate_type=None, aggregate_interval=None,
        ...
        if aggregate_type:
            ...
            sql_str = 'SELECT dateTime, %s, %s, usUnits, %s FROM %s ' \
                      'WHERE dateTime >= ? AND dateTime <= ?' \
                      % (WindVec.windvec_types[obs_type][0], WindVec.windvec_types[obs_type][1],
                         db_manager.quote('interval'), db_manager.table_name)
            std_unit_system = None
